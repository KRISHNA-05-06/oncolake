...
An **orchestration** pipeline loads from S3 and then chains into a **transformation** pipeline, so one end-to-end run does everything. Matillion authenticates to AWS via cloud credentials and to Snowflake via the project connection; the JDBC source credential is read from AWS Secrets Manager rather than hardcoded.
...
The tuning experiment produced a genuinely useful finding, described below.
